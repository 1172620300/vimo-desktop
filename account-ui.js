(()=>{
 ...
 overlay.innerHTML='<div class="account-panel"><div class="account-identity"><div class="account-mark" aria-hidden="true">V</div><div><h1>'+poolName+'</h1><p class="account-subtitle">使用创作端独立账号登录</p></div></div><form><label>账号<input name="username" placeholder="请输入账号" autocomplete="username" required minlength="3" maxlength="40" spellcheck="false"></label><label>密码<input name="password" placeholder="请输入密码" type="password" autocomplete="current-password" required minlength="12" maxlength="200"></label><label class="account-remember"><input name="remember" type="checkbox" checked> 记住登录（7天）</label><button type="submit">登录</button><div class="account-message" role="alert"></div></form><p class="account-note">账号由管理员创建 · 与 Worker 账号独立</p></div>';
 ...
 function lock(){window.vimoSignedIn=false;window.dispatchEvent(new Event('account-locked'));overlay.hidden=false;overlay.style.display='grid';bar.hidden=true;for(const child of document.body.children)if(child!==overlay&&child!==bar)child.inert=true;}
 function unlock(user){
  ...
  window.dispatchEvent(new CustomEvent('account-ready',{detail:{id:user.id}}));
 }
